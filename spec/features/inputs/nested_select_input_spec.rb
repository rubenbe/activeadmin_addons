...
describe "Nested Select Input", type: :feature do
  ...
  context "with nested resources" do
    ...
    it "sets new values correctly", js: true do
      ...
      on_nested_ctx(1) do
        on_input_ctx("#{prefix}0_country_id"){
          open_select2_options
          click_select2_option(@chile.id)
        }
        on_input_ctx("#{prefix}0_region_id"){
          open_select2_options
          click_select2_option(@metropolitana.id)
        }
        on_input_ctx("#{prefix}0_city_id"){
          open_select2_options
          click_select2_option(@santiago.id)
        }
        expect_nested_select2_result_text_to_eq(3, @santiago.id)
      end

      click_add_nested
      on_nested_ctx(2) do
        on_input_ctx("#{prefix}1_country_id"){
          open_select2_options
          click_select2_option(@argentina.id)
        }
        on_input_ctx("#{prefix}1_region_id"){
          open_select2_options
          click_select2_option(@cuyo.id)
        }
        on_input_ctx("#{prefix}1_city_id"){
          open_select2_options
          click_select2_option(@mendoza.id)
        }
        expect_nested_select2_result_text_to_eq(3, @mendoza.id)
      end
